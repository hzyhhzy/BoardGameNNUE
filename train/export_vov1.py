...
if __name__ == '__main__':
    ...
    if os.path.exists(file_path):
        ...
        if(model_type != "vov1"):
            ...
            exit(0)
        model = Model_vov1(*model_param).to(device)
    ...
    time0=time.time()
    feature_c, mlpc1, mlpc2, _, _, _ = model_param
    print(f"Start: feature_c={feature_c}, mlpc1={mlpc1}, mlpc2={mlpc2}")
    exportPath='../export/'+exportname+'.txt'
    exportfile=open(exportPath,'w')

# file head
# -------------------------------------------------------------
    print("vov1",file=exportfile)
    print(feature_c,mlpc1,mlpc2,end=' ',file=exportfile)
...
# -------------------------------------------------------------
    ...
    for i in range(mlpc1):
        for j in range(mlpc2):
            print(w[j][i],end=' ',file=exportfile)
    print('',file=exportfile)

    print("mlp_b2",file=exportfile)
    for i in range(mlpc2):
        print(b[i],end=' ',file=exportfile)
    print('',file=exportfile)
